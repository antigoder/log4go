...
var (
	levelStrings = [...]string{"FNST", "FINE", "DEBG", "TRAC", "INFO", "WARN", "ERR", "CRIT"}
)
...
func (log Logger) intLogf(lvl Level, format string, args ...interface{}) {
	...
	src := ""
	if ok {
    src = fmt.Sprintf("%s:%d %s()", path.Base(filename), lineno, runtime.FuncForPC(pc).Name())
	}
...
func (log Logger) intLogc(lvl Level, closure func() string) {
	...
	src := ""
	if ok {
		src = fmt.Sprintf("%s:%d %s()", path.Base(filename), runtime.FuncForPC(pc).Name(), lineno)
	}
